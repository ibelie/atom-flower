...
class FlowerTree {
	...
	/**
	 * Attaches the FlowerTree to the DOM.
	 *
	 * @param  {HTMLElement} [parent] the DOM node where attaching the FlowerTree
	 */
	attach(parent) {
		if (this.attached) { return; }

		const container = parent || atom.views.getView(this.flower.editor);
		let trees = container.querySelectorAll('atom-flower-tree');
		if (trees.length) {
			Array.prototype.forEach.call(trees, (el) => el.destroy());
		}
