...
class FlowerView {
	 ...
	 */
	attach(parent) {
		let views = parent.querySelectorAll(this.tagName.toLowerCase());
		if (views.length) {
			Array.prototype.forEach.call(views, (el) => el.style.display = 'none');
		}
		if (!this.attached) {
...
module.exports = {
	...
	Register: (name, View) => {
		...
		FlowerModel.View = registerOrUpdateElement('atom-flower-' + name, {
